...
def heuristic(a, b):
    # Euclidean distance for diagonal-friendly A*
...
def astar(maze):
    ...
    heap = [(heuristic(start, goal), 0, 0, start[0], start[1])]
    visited = {}

    while heap:
        priority, steps, coins, r, c = heapq.heappop(heap)
        if (r, c) == goal:
            return coins
        for dr, dc in dirs:
            nr, nc = r + dr, c + dc
            if 0 <= nr < len(maze) and 0 <= nc < len(maze[0]) and maze[nr][nc] != 'X':
                ...
                if (nr, nc) not in visited or \
                   (steps + 1 < visited[(nr, nc)][0]) or \
                   (steps + 1 == visited[(nr, nc)][0] and new_coins < visited[(nr, nc)][1]):
                    visited[(nr, nc)] = (steps + 1, new_coins)
                    est_cost = steps + 1 + heuristic((nr, nc), goal)
                    heapq.heappush(heap, (est_cost, steps + 1, new_coins, nr, nc))
    return None
